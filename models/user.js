...
const userSchema = new Schema({
  ...
  }//reference ot a product inside items
...
class User{
  ...
  addToCart(product){
    ...
    if(cartProductIndex>=0){
      ...
      updatedCartItems.push({ productId: new ObejctId(product._id) , quantity :newQuantity})
    }
  ...
  deleteItemFromCart(productId){
   ...
   // const updatedCart = {items: updatedCartItems};
    const db = getDb();
    return db.collection('users').updateOne({_id: new ObejctId(this._id)}, {$set:{cart:{items:updatedCartItems} }})
  ...
  static findById(userId){
    ...
    return db.collection('users').findOne({_id: new ObejctId(userId)}).then(user=>{
      console.log(user);
      return user;
